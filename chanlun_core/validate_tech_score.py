...
def compute_technical_score(daily_analyzer, m30_analyzer, buy_point) -> dict:
    ...
    if not daily_analyzer.zhongshus:
        ...
        # 检查是否有向上一笔（一买确认）
        # v5.4.1(AUD-C-06): buy_point.date 为 None(scanner 缓存 buy_date 解析
        # 失败的合法形态)时旧实现 datetime>None TypeError → 精细评分被外层吞掉
        # 静默塌缩为粗分。显式跳过该子项并留痕。
        has_up_after = False
        if hasattr(buy_point, 'date') and buy_point.date is not None:
            has_up_after = any(
                b.direction == 'up' and b.start_date > buy_point.date
                for b in daily_analyzer.bis
            )
        elif hasattr(buy_point, 'date') and buy_point.date is None:
            details.append("买点日期缺失: 跳过确认笔检查(不扣分)")
    ...
    # 向上一笔确认
    # v5.4.1(AUD-C-06): date=None 防御(同 133 段)——跳过比较不抛 TypeError
    has_up_after = False
    if hasattr(buy_point, 'date') and buy_point.date is not None:
        has_up_after = any(
            b.direction == 'up' and b.start_date > buy_point.date
    ...
    if (hasattr(buy_point, 'date') and buy_point.date is not None
            and daily_analyzer.klines and len(daily_analyzer.klines) > 20):
