...
# v5.4.1(AUD-A-06): 单飞缓存包装——三维编排中 negative 检查与本法共享同一次
# 采集结果, 降级路径(iwencai失败→多源)不再双跑八源+双LLM
from news_scanner import scan_news_cached as _ns_scan_news_cached
...
def analyze_single_stock(code: str, name: str, output_path: str = "") -> dict:
    ...
    # v5.4.1(AUD-A-06): 走单飞缓存——同进程内 negative 检查复用本次结果
    score, detail = _ns_scan_news_cached(code, name)
