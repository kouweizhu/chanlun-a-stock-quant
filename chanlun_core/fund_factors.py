...
def _flow_rows(code: str) -> tuple[list, str]:
    ...
    v5.4.1(AUD-C-01): 出口统一按 date 升序(旧→新)归一化——新浪源 asc=0
    降序出新在前，曾致消费端 rows[-20:] 切出最旧20日窗口(最近10日缺席)、
    reversed streak 方向颠倒。源头(em_utils.fund_flow_backup)已固化升序，
    此处做防御性二次归一，消费方无需再关心行序。"""
    try:
        rows = em_utils.stock_fund_flow_120d(code)
        if rows:
            return _sorted_by_date(rows), "em"
    except Exception:
        pass
    try:
        # 新浪备用源字段: {date, close, net_amount, turnover}
        rows = em_utils.fund_flow_backup(code, days=30)
        return (_sorted_by_date(rows), "sina") if rows else ([], "sina")
    except Exception:
        return [], "sina"


def _sorted_by_date(rows: list) -> list:
    """按 date 升序稳定排序（date 格式 YYYY-MM-DD 字符串序即时间序）。"""
...
def compute_fund_factor_score(code: str) -> dict:
    ...
    # v5.4.1(AUD-C-05): 单因子噪声直通护栏——与 alpha_factor_filter C-02
    # "仅剩单因子的股不得直接定生死"同哲学。旧实现单因子以满权 1.0 决定整个
    # 资金面分(如仅剩 holder 且某季环比+6% → 35分直进五维复合), 信息量不足。
    # 有效子因子==1 → 返回中性50 + degraded 标记宁缺勿错。
    single_factor_degraded = (
        sum(1 for v in available.values() if v is not None) == 1
        and not all(v is None for v in available.values())
    )
    if single_factor_degraded:
        fund_factor = 50.0
    else:
        fund_factor = 0.0
    ...
    return {
        ...
        **({"degraded_single_factor": True} if single_factor_degraded else {}),
    }
