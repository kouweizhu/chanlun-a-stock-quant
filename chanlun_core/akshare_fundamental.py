...
def scan_research_reports(symbol: str, lookback_days: int = 30) -> tuple:
    ...
    try:
        ...
        # 3. 盈利预测 (EPS 预期)
        # v5.4.1(P3收尾): 变量/标签动态年份化——"eps_2026"/"2026EPS预测"标签
        # 在 2027 年起语义过期(取的其实是当年列, 标签却印死 2026)
        _cur_yr = datetime.now().year
        eps_forecast = None
        # v5.4(B-09): 动态年份列——硬编码"2026-盈利预测"2027年起永远取不到值
        _forecast_col = f"{_cur_yr}-盈利预测-收益"
        for col in [_forecast_col]:
            if col in recent.columns:
                val = recent[col].dropna()
                if not val.empty:
                    try:
                        eps_forecast = float(val.iloc[0])
                    except (ValueError, TypeError):
                        pass

        if eps_forecast is not None and eps_forecast > 0:
            details_parts.append(f"{_cur_yr}EPS预测={eps_forecast:.2f}")
