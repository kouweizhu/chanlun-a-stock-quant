...
def generate_html(symbol, name=None, shared_recsys=None):
    """生成HTML报告

    v5.4.1(AUD-A-05): shared_recsys 提供时(编排器 chanlun 线程已完成的
    RecursiveTimingSystem)直接复用其 daily/30min 分析结果渲染, 不再二次
    全量分析。独立 CLI 调用不传该参, 行为与旧版一致。"""
    if name is None:
        name = STOCK_NAMES.get(symbol, symbol)

    if shared_recsys is not None:
        rec_sys = shared_recsys
        daily = rec_sys.analyses.get('daily')
    else:
        dm = DataManager()
