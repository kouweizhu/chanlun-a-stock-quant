...
def run(cmd, desc, timeout=1200):
    ...
    try:
        result = subprocess.run(parts, cwd=BASE, capture_output=True,
                              text=True, encoding="utf-8", errors="replace",
                              timeout=timeout, env=env)
    except subprocess.TimeoutExpired as e:
        raise RuntimeError(f"{desc} 超时({timeout}s)，中止全流程") from e
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"{desc} 失败 (exit={result.returncode})，中止全流程防止陈旧数据流入报告")
    return result
...
def resocre_with_alpha():
    ...
    for s in scored:
        ...
        s['position_pct'] = f"{result.position*100:.0f}%"
    ...
    # 重新生成报告（但 HTML 会因缺少 analyzer 失败，没关系）
    print(f"\n{'='*40}")
    print("重新生成四维 MD 报告 + 汇总表")
    print(f"{'='*40}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 2. Phase 2+3（生成 HTML + MD，analyzers 在内存）
        run("python pool_screener.py --from-cache", "Phase 2+3: 评分 + 报告生成", timeout=1800)
