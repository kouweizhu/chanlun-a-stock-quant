...
def compute_veto_check(chanlun_data: dict, fund_data: dict, negative_data: dict,
    ...
    if news_data:
        # v5.4(B-10): 旧实现读 news_data["relevant_lines"]——该键是渲染期
        # extract_news_details 的产物, veto 阶段根本不存在, 分支恒空形同虚设
        # (审计 B-10)。改读真实随行的 detail 多行文本(本股新闻采集结果,
        # 条目天然个股级), 非空行参与 severe 关键词匹配。
        _detail_txt = str(news_data.get("detail") or "")
        _sev_texts.extend(ln.strip() for ln in _detail_txt.splitlines() if ln.strip())
...
| 估值水平 | **{{ fund.fundamental_score.valuation_score }}** | PE~{{ '%.1f'|format(fund.valuation.peTTM) if fund.valuation.peTTM else '' }} |
...
def main():
    ...
    if args.input:
        ...
        r = subprocess.run(
            ["python", ssa_path, "--code", code, "--name", name, "--output", "/tmp/_ssa_temp.json"],
            capture_output=True, text=True, timeout=300
        )
        if r.returncode != 0:
            print(f"❌ 分析失败: {r.stderr[:500]}", file=sys.stderr)
            sys.exit(1)
        with open("/tmp/_ssa_temp.json", "r", encoding="utf-8") as f:
            result = json.load(f)
    else:
        parser.print_help()
        sys.exit(1)
