...
def search_negative(code: str, name: str, hours: int = 24) -> dict:
    ...
    if _HAS_NEWS_SCANNER:
        try:
            _score, detail = _ns_scan_news(code, name)
...
def main():
    ...
    for code, name, industry in monitor:
        if args.full and _HAS_NEWS_SCANNER:
            ...
            src = detail.split(chr(10))[0] if detail else "多源"
            # 解析消息明细，提取负面消息
            msg_lines = detail.split(chr(10))[1:] if chr(10) in detail else []
            for msg_line in msg_lines:
                is_neg = "负面" in msg_line or "混合" in msg_line
                if is_neg:
                    # 从 "[来源][标签] 标题" 中提取标题
                    parts = msg_line.split("] ", 1)
                    title = parts[1] if len(parts) > 1 else msg_line
                    level = "L3" if any(kw in msg_line for kw in ["立案调查", "立案侦查", "造假", "退市", "暴雷"]) else \
                            "L2" if any(kw in msg_line for kw in ["减持", "处罚", "诉讼", "制裁", "立案", "冻结"]) else "L1"
                    all_news.append({
                        "symbol": code, "name": name, "industry": industry,
                        "title": title[:80], "summary": "",
                        "date": title[:10] if title[:10].count("-") == 2 else "",
                        "source": src, "level": level, "neg_hits": [],
                    })
            if not args.json:
                neg_count = len(all_news) - sum(1 for n in all_news if n["symbol"] != code)
                status = f"✅ {src} ({score}分)" if score >= 50 else f"⚠ {src} ({score}分)"
