...
def calculate_fundamental_score(data, multi_year_data=None, valuation_percentiles=None):
    """计算基本面四维度评分（各25分，总分100）+ 趋势修正
    支持传入 multi_year_data 做趋势评分和ROE标准差计算。
    valuation_percentiles: 可选 {"pe_p": float, "pb_p": float} 估值历史分位（0-100），
        用于估值评分额外惩罚（v6.2 新增，2026-08-29）。
    """
    scores = {
        ...
        "valuation_percentiles": valuation_percentiles or {},
    }
    ...
    # ---- 估值历史分位惩罚（v6.2 新增，2026-08-29） ----
    # 当 PB/PE 处于历史极高分位时，当前绝对估值参考意义有限，额外惩罚以反映泡沫风险。
    _vp = valuation_percentiles or {}
    _pe_p = _vp.get("pe_p")
    _pb_p = _vp.get("pb_p")
    _penalty = 0
    _penalty_parts = []
    if _pb_p is not None:
        if _pb_p > 90:
            _penalty += 3
            _penalty_parts.append(f"PB分位{_pb_p:.0f}%>90%→-3")
        elif _pb_p > 75:
            _penalty += 1
            _penalty_parts.append(f"PB分位{_pb_p:.0f}%>75%→-1")
    if _pe_p is not None:
        if _pe_p > 90:
            _penalty += 2
            _penalty_parts.append(f"PE分位{_pe_p:.0f}%>90%→-2")
        elif _pe_p > 75:
            _penalty += 1
            _penalty_parts.append(f"PE分位{_pe_p:.0f}%>75%→-1")
    if _penalty > 0:
        v_score -= _penalty
        scores["valuation_penalty_detail"] = "；".join(_penalty_parts)

    scores["valuation_score"] = max(0, min(10, v_score))
