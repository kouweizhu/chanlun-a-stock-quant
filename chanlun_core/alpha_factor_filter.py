...
def merge_into_phase2(alpha_scores: dict[str, float],
    ...
    Args:
        ...
        path: phase2_results 路径（默认 chanlun_core/.phase2_results.json；
              可注入便于单元测试，regression_check 使用）
    """
    phase2_path = Path(path) if path is not None else Path(__file__).parent / ".phase2_results.json"
    ...
    for item in phase2:
        ...
        if code in score_map:
            c = score_map[code]
            # v5.4.1(AUD-B-01·2026-08-27): 增量合并而非整键覆写——候选来自
            # scanner 缓存(天然无 veto/severe 键→恒[])，旧实现无条件赋值会把
            # Phase2 风控增强(pool_screener risk_enhance 解禁预警)写入的
            # severe_reasons 洗成[]，M-02 无条件重算放大后该股被当干净标的
            # 重建仓位(实证 composite 50.8→71.7、仓位15%→30%)，
            # severe -15分/限轻仓通道静默失守。
            item["veto_reasons"] = sorted(
                set(item.get("veto_reasons") or []) | set(c.get("veto_reasons") or []))
            item["severe_reasons"] = sorted(
                set(item.get("severe_reasons") or []) | set(c.get("severe_reasons") or []))
