...
def write_record(data):
    ...
    # v5.4.1(AUD-C-04): composite_score 缺失显式报错——旧实现静默写 NULL 行,
    # 随后 query/trend/list 格式化 None 直接 TypeError 崩溃整批展示(B-07 后半)。
    # 如调用方确无综合分, 请显式传 composite_score=None 之外的数值或先自行兜底。
    if data.get("composite_score") is None and "composite_score" not in data:
        raise ValueError(
            "write_record: 缺少必需键 composite_score (B-07契约)。"
            f"收到键: {sorted(data.keys())}")
...
def query_stock(code, limit=5):
    ...
    print("-" * 62)
    def _fmt(v, w=6):
        # v5.4.1(AUD-C-04): NULL 行防御——毒行显示占位不再 TypeError
        return f"{v:>{w}.1f}" if v is not None else "—" * min(w, 3)
    for r in rows:
        print(f"{r['report_date']:<12} {_fmt(r['composite_score'])} {_fmt(r['tech_score'])} "
              f"{_fmt(r['fund_score'])} {_fmt(r['news_score'])} "
              f"{r['decision'] or '-':<10} {r['position_suggestion'] or '-':<10}")
...
def list_recent(limit=10):
    ...
    for r in rows:
        ...
        cs_s = f"{cs:>6.1f}" if cs is not None else "—"
        print(f"{r['stock_code']:<8} {(r['stock_name'] or ''):<10} {r['report_date']:<12} "
              f"{cs_s:>6} {r['decision'] or '-':<10}")
...
def show_trend(code):
    ...
    for r in rows:
        cs = r['composite_score']
        change = ""
        if prev is not None and cs is not None:
            diff = cs - prev
            change = f"{'+' if diff > 0 else ''}{diff:.1f}"
        print(f"{r['report_date']:<12} {_fmt(cs)} {change:>7} {_fmt(r['tech_score'])} "
              f"{_fmt(r['fund_score'])} {_fmt(r['news_score'])} {r['decision'] or '-':<10}")
        prev = cs
