...
def _run_html(code: str, name: str) -> dict:
    """HTML可视化报告"""
    mod = _import_html()
    return mod.generate_html(code, name)
...
def analyze(code: str, name: str = None, timeout_per_module: int = 180) -> dict:
    ...
    not_done = set()
    try:
        for module_name, (fn, *args) in tasks.items():
            # v5.4(B-15): 捕获移入工作函数体内（_run_captured）
            future_map[pool.submit(_run_captured, fn, *args)] = module_name

        done, not_done = wait(future_map.keys(), timeout=timeout_per_module)
        if not_done:
            grace = max(30, timeout_per_module // 2)
