...
def sina_financial_report(code: str, report_type: str = "lrb", num: int = 8) -> list[dict]:
    ...
    # v5.4.1(P3·AUD-C卫生): 旧实现引用未导入的 requests → 一调用即 NameError,
    # L3 新浪三表兜底实际永久失效(被上游 B-21 守卫吞成降级)。改标准库 urllib。
    full_url = url + "?" + urllib.parse.urlencode(params)
    req = urllib.request.Request(full_url, headers={"User-Agent": UA, "Referer": "https://finance.sina.com.cn/"})
    with urllib.request.urlopen(req, timeout=15) as _resp:
        r_json = json.loads(_resp.read().decode("utf-8", "ignore"))
    report_list = r_json.get("result", {}).get("data", {}).get("report_list", {}) or {}
    rows = []
...
def fund_flow_backup(code: str, days: int = 60) -> list:
    ...
    行序契约(v5.4.1/AUD-C-01): 按 date 升序(旧→新)——与 stock_fund_flow_120d
    (push2his daykline 天然升序)同口径。新浪接口本身 asc=0 降序出新在前，
    曾致消费端 rows[-20:] 切出最旧窗口+streak 方向颠倒（审计0827 P1），
    此处在源头固化排序，勿改回。"""
    code = norm_ticker(code)
    ...
    rows = [{"date": x.get("opendate"), "close": x.get("trade"),
             "net_amount": x.get("netamount"), "turnover": x.get("turnover")} for x in arr]
    # 源为降序(新→旧) → 反转固化升序(旧→新)出口契约
